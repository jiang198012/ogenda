...
| **提醒通知** | 提前 5 分钟到 1 天可选,VALARM 与 CalDAV 互通,到点弹通知(可关闭) |
...
表单「提醒」下拉可选提前量(不提醒 / 开始时 / 5–30 分钟 / 1 小时 / 1 天),写入 CalDAV 的 VALARM,手机端日历也能收到。**设置 → Ogenda → 提醒**里打开「启用事件提醒」后,Obsidian 打开期间到点弹通知(默认每 30 秒检查一次);可设置新事件的默认提醒。
