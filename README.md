...
- **v1.0.3** — **月视图高密度不再卡死**:单日事件超过 6 条时折叠为「还有 N 个」,点击展开,整月最后一周始终可滚动到底;周视图窄侧边栏下 7 列完整显示(不再裁掉周末);日视图的回复状态显示本地化文案(如「已接受」而非 ACCEPTED)。
- **v1.0.2** — **窄侧边栏不再溢出**:日/月视图改用容器查询,跟随面板自身宽度响应。拖窄 Obsidian 侧边栏时,日视图自动纵向堆叠、月视图 7 列完整显示、导航栏自动换行——不再有文字溢出。
