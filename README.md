...
| **五种视图** | 列表 / 日 / 周 / 月 / 统计仪表盘 |
| **事件编辑器** | 分类标签、状态、地点、组织者、参与者、重复规则、提醒 |
| **重复事件** | 表单预设每天/每周/每月等规则,支持自定义 RRULE;点击实例可选「仅本次 / 全部 / 跳过本次」 |
| **提醒通知** | 支持多个带单位的提前时间,写入多个 VALARM 与 CalDAV 互通,到点弹通知(可关闭) |
| **日视图时间格** | 00:00–24:00 时间格:点击空白新建、拖拽划范围、拖卡片移动、拖底部调时长 |
| **周视图时间格** | 7 列 × 24 小时:事件按时间定位,与日视图一致的拖拽操作,支持跨列拖动 |
| **时间线分区** | 设置里自定义时间段与填充色,日/周视图时间格铺半透明色带,一眼看清时间区间 |
| **24 小时制时间输入** | 告别 `12:00 AM` 歧义;`1423`、`900` 边打边格式化,键盘输入丝滑 |
| **自然语言快速创建** | 命令「快速新建事件」:一句话解析出日期/时间/时长/标题,如 `明天下午3点 和经理开会` |
...
- 唯一的例外:**提醒**。在设置里打开「启用事件提醒」后,Obsidian 打开期间每 30 秒检查一次是否有到点的提醒——开关默认关闭。
...
运行命令 **"Ogenda: Quick add event"**,一句话描述事件,回车自动填入表单:

- `明天下午3点 和经理开会` → 明天 15:00,时长 1 小时
- `周五 10:00 周会 1小时` → 本周五 10:00–11:00
- `tomorrow 3pm standup 30min` → 英文也可以
- 也支持 `今天`/`后天`/`下周一`/`晚上8点半`/`7-25` 等写法;没提到日期/时间时默认今天 09:00。
...
- 点击空白格 → 在该时刻新建;拖拽空白格 → 划出时间段新建
- 拖动事件卡片 → 移动;拖卡片**底部边缘** → 调整结束时间
- 全天事件显示在顶部横条;当前时刻有红色指示线

### 周视图:时间格

周视图与日视图同一套时间格(7 列 × 24 小时):事件按真实时间定位、重叠自动分列;点击/拖拽/移动/改时长与日视图一致;把卡片拖到另一天即可改日期。

**周视图与日视图都只展示设置了分区的时段**(默认 06:00–23:00):清晨之前和深夜不占空间,一眼看到的都是你的活动时间;窗口外的事件不显示。

### 时间线分区

设置 → Ogenda → **时间线分区**:添加/删除时间段,每段设名称、起止时间(24 小时制,结束可早于开始表示跨午夜)和填充色。分区以半透明色带铺在日/周视图的时间格里(默认 14% 透明度),事件仍清晰可读;取消勾选或删光即回到无分区样式。默认提供六段:清晨 06:00–08:30 / 上午 08:30–12:00 / 中午 12:00–14:00 / 下午 14:00–17:00 / 傍晚 17:00–20:00 / 晚上 20:00–23:00。

### 新建与编辑事件

点击 **New event(新建事件)** 或已有事件卡片进行编辑。支持分类标签、状态、地点、组织者、参与者、重复规则、提醒。

### 重复事件

表单里选「重复」(每天 / 每个工作日 / 每周 / 每月 / 每年 / 自定义)。点击重复事件的**某个实例**时,会先问:

- **仅本次** — 把本次改成一个独立事件,并在原序列里排除原时间
- **全部** — 编辑整个序列
- **跳过本次** — 只排除本次,不新建

### 提醒

表单「提醒」每行填写一个带单位的提前时间,点击「添加提醒」即可增加多次提醒(如 `1天`、`1小时`、`30分钟`);也兼容直接输入逗号分隔的旧格式。写入 CalDAV 的多个 VALARM,手机端日历也能收到。旧的纯数字分钟格式仍可读取。**设置 → Ogenda → 提醒**里打开「启用事件提醒」后,Obsidian 打开期间到点弹通知(默认每 30 秒检查一次);可设置新事件的默认提醒。
...
| 提醒 | 启用事件提醒 | 到点弹通知(每 30 秒检查一次) | 关 |
| | 默认提醒 | 新建事件的默认提前量(可在表单覆盖) | 不提醒 |
| 时间线分区 | 时间段列表 | 每段:启用开关 + 名称 + 起止时间 + 填充色,可增删 | 清晨/上午/中午/下午/傍晚/晚上 6 段 |
...
- **v1.3.0** — **视图跟随分区窗口,按情况自适应**:日/周视图只展示设置了分区的时段(默认 06:00–23:00),但深夜有事件时时间轴自动延伸显示;**周视图贯通时间线**:06:00/12:00/18:00 三条线横贯整周 + 列内小时线;事件标签统一为只显示标题的紧凑形式;默认六段分区换新色卡(相邻色相 ≥44°,一眼可辨);修复事件块与色带错位、贯通线对齐等。
- **v1.2.0** — **时间线分区**:设置里自定义时间段与填充色(支持跨午夜),日/周视图时间格铺半透明色带;周视图升级为 7 列 × 24 小时时间格,事件按真实时间定位,拖拽操作与日视图一致。
- **v1.1.0** — **提醒通知**:事件可设提前提醒(VALARM 与 CalDAV 互通),设置里开启后到点弹通知;**重复事件编辑**:表单预设重复规则 + 自定义 RRULE,点击实例可选「仅本次 / 全部 / 跳过本次」(EXDATE 双向同步);**日视图时间格 + 拖拽**:点空白新建、拖拽划范围、拖卡片移动、拖底部改时长,周/月视图可跨日拖动;**自然语言快速创建**:`明天下午3点 和经理开会` 一句话建事件。
